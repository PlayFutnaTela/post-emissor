...
if (!defined('POST_EMISSOR_VERSION')) {
    define('POST_EMISSOR_VERSION', '2.0.0');
}
...
if (!defined('POST_EMISSOR_SLUG')) {
    define('POST_EMISSOR_SLUG', 'post-emissor');
}

/**
 * Ativação do plugin
 */
function post_emissor_activate() {
    // Inicializar instância principal para criar tabelas
    require_once POST_EMISSOR_DIR . 'includes/class-emissor-core.php';
    $emissor = Post_Emissor_Core::get_instance();
    $emissor->create_database_tables();
    
    // Agendar eventos do WP-Cron
    if (!wp_next_scheduled('post_emissor_process_queue')) {
        wp_schedule_event(time(), 'every_minute', 'post_emissor_process_queue');
    }
}
register_activation_hook(__FILE__, 'post_emissor_activate');

/**
 * Desativação do plugin
 */
function post_emissor_deactivate() {
    wp_clear_scheduled_hook('post_emissor_process_queue');
}
register_deactivation_hook(__FILE__, 'post_emissor_deactivate');

/**
 * Desinstalação do plugin
 */
function post_emissor_uninstall() {
    global $wpdb;
    
    // Opções a serem removidas
    $options = array(
        'post_emissor_receivers',
        'post_emissor_origin_language',
        'post_emissor_encryption_key'
    );
    ...
    $tables = array(
        $wpdb->prefix . 'post_emissor_logs',
        $wpdb->prefix . 'post_emissor_queue',
        $wpdb->prefix . 'post_emissor_reports',
        $wpdb->prefix . 'post_emissor_receivers'
    );
...
register_uninstall_hook(__FILE__, 'post_emissor_uninstall');
