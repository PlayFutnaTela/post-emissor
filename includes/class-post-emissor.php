...
class Post_Emissor {
    ...
    public function handle_save_post($post_ID, $post, $update) {
        ...
        if ('publish' === $post->post_status) {
            // Antes coletávamos $results e set_transient(...). Agora removemos:
            $this->send_post_data($post);
        }
    }
     ...
     *
     * Agora removemos o relatório e não retornamos nada,
    ...
    private function translate_text_context($text, $source_lang, $target_lang, $context = 'default') {
        ...
        // $clean_text = $text; // Inicializa $clean_text com o texto processado até agora
        // // Loop para remover todos os spans, incluindo aninhados
        // $max_iterations = 10; // Limite para evitar loops infinitos
        // $iteration = 0;
        // while (preg_match('/<span\b[^>]*>/i', $clean_text) && $iteration < $max_iterations) {
        //     // Remove spans com a classe _fadeIn_m1hgl_8
        //     $clean_text = preg_replace('/<span\b[^>]*class=["\'][^"\']*_fadeIn_m1hgl_8[^"\']*["\'][^>]*>(.*?)<\/span>/is', '$1', $clean_text);
        //     // Remove qualquer outro span
        //     $clean_text = preg_replace('/<span\b[^>]*>(.*?)<\/span>/is', '$1', $clean_text);
        //     $iteration++;
        // }
        
        // Verifica se ainda há spans remanescentes
        // if (preg_match('/<span\b[^>]*>/i', $clean_text)) {
        //     error_log('Post Emissor - Aviso: Não foi possível remover todos os spans após ' . $max_iterations . ' iterações.');
        // }
        ...
        switch ($context) {
            case 'title':
                ...
                break;
            case 'body':
                // $custom_prompt = "$fluency_statement Translate the following content from {$source_name} to {$target_name}, preserving the meaning and context while ignoring and removing any HTML markup (for example span, p, text and others) or placeholders like %%SHORTCODE%% in the translation process.";
        ...
        for ($attempt = 0; $attempt < $max_retries; $attempt++) {
            ...
            if ( !is_array($data) || !isset($data['choices']) || !is_array($data['choices']) || empty($data['choices']) || !isset($data['choices'][0]['message']['content']) ) {
                error_log('Post Emissor - Resposta inválida da API OpenAI (' . $context . ') na tentativa ' . ($attempt+1) . ': ' . print_r($data, true));
                continue;
            }
